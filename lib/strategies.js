...
var strategies = {
  bitbucket: require('./BitBucketStrategy'),
  github: require('./GitHubStrategy'),
  gitlab: require('./GitLabStrategy')
}

function factory(provider, headers) {
  if((Object.keys(strategies)).indexOf(provider) === -1) {
    throw new Error("Unkwnown provider: " + provider);
  }
  return new strategies[provider](headers);
}
